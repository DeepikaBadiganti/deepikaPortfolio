...
const Modal = ({onClose,toggle}) => {
    return createPortal(
      <div className="fixed inset-0 bg-background/60 backdrop-blur-sm flex items-center justify-center">
        <div
          className="bg-background/20 border border-cyan-300/60 border-solid backdrop-blur-[6px]
        ...
        >
          ...
          <div className="flex items-center justify-center space-x-4">
            <button
              onClick={toggle}
              className="px-4 py-2 border border-cyan-300/60 border-solid hover:shadow-glass-sm rounded mr-2"
            >
              Yes
            </button>
            <button
              onClick={onClose}
              className="px-4 py-2 border border-cyan-300/60 border-solid hover:shadow-glass-sm rounded "
            >
              No
            </button>
          </div>
...
const Sound = () => {
  ...
  useEffect(() => {
    ...
    if (consent && consentTime && new Date(consentTime).getTime() + 1 * 24 * 60 * 60 * 1000 > new Date() ){
  ...
  return (
    <div className="fixed top-4 right-2.5 xs:right-4 z-50 group">
      ...
      <audio ref={audioRef} loop>
        <source
          src={"/audio/waterfall-underwater-epic-stock-media-2-2-00-32.mp3"}
          type="audio/mpeg"
        />
